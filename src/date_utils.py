...
def get_day_name(input_date):
    ...
    if isinstance(input_date, str):
        try:
            ...
            raise ValueError(f"Invalid date string. Use YYYY-MM-DD format. Received: {input_date}")
    
    # Validate input is a date object
    if not isinstance(input_date, date):
        raise TypeError(f"Input must be a date object or YYYY-MM-DD formatted string. Received: {type(input_date)}")
